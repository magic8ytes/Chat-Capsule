...
const logger = createElectronLogger('ImageDecryptService')
...
export class ImageDecryptService {
  ...
  async resolveCachedImage(payload: { sessionId?: string; imageMd5?: string; imageDatName?: string }): Promise<DecryptResult & { hasUpdate?: boolean }> {
    ...
    for (const key of cacheKeys) {
      const hdCached = this.findCachedOutput(key, true, payload.sessionId)
      if (hdCached && !this.isThumbnailPath(hdCached) && existsSync(hdCached) && this.isImageFile(hdCached)) {
        this.cacheResolvedPaths(key, payload.imageMd5, payload.imageDatName, hdCached)
        const dataUrl = this.fileToDataUrl(hdCached)
        const localPath = dataUrl || this.filePathToUrl(hdCached)
        this.updateFlags.delete(key)
        this.emitCacheResolved(payload, key, localPath, false)
        return { success: true, localPath, hasUpdate: false, isThumb: false }
      }
    }
    ...
    for (const key of cacheKeys) {
      ...
      if (cached && existsSync(cached) && this.isImageFile(cached)) {
        ...
        this.emitCacheResolved(payload, key, localPath, isThumb)
        return { success: true, localPath, hasUpdate, isThumb }
      }
  ...
  async decryptImage(payload: { sessionId?: string; imageMd5?: string; imageDatName?: string; force?: boolean }): Promise<DecryptResult> {
    ...
    if (!payload.force) {
      ...
      if (cached && existsSync(cached) && this.isImageFile(cached)) {
        ...
        const localPath = dataUrl || this.filePathToUrl(cached)
        const isThumb = this.isThumbnailPath(cached)
        this.emitCacheResolved(payload, cacheKey, localPath, isThumb)
  ...
  private async resolveHardlinkPath(accountDir: string, md5: string, _sessionId?: string): Promise<string | null> {
    try {
      ...
      const rowResult = await wcdbReadonlySqlGateway.execQuery(
        'media',
        hardlinkPath,
      ...
      if (state.dirTable) {
        try {
          // 通过 rowid 查询目录名
          const dir1Result = await wcdbReadonlySqlGateway.execQuery(
            'media',
            hardlinkPath,
          ...
          const dir2Result = await wcdbReadonlySqlGateway.execQuery(
            'media',
            hardlinkPath,
  ...
  private async getHardlinkState(accountDir: string, hardlinkPath: string): Promise<HardlinkState> {
    ...
    const imageResult = await wcdbReadonlySqlGateway.execQuery(
      'media',
      hardlinkPath,
      "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'image_hardlink_info%' ORDER BY name DESC LIMIT 1"
    )
    const dirResult = await wcdbReadonlySqlGateway.execQuery(
      'media',
      hardlinkPath,
  ...
  private async ensureWcdbReady(): Promise<boolean> {
    if (wcdbConnectionGateway.isReady()) return true
    const dbPath = this.configService.get('dbPath')
    const decryptKey = this.configService.get('decryptKey')
    const wxid = this.configService.get('myWxid')
    const usesMacBackend = wcdbConnectionGateway.isMacBackendActive()
    if (!dbPath || !wxid || (!decryptKey && !usesMacBackend)) return false
    const cleanedWxid = this.cleanAccountDirName(wxid)
    return await wcdbConnectionGateway.open(dbPath, decryptKey || '', cleanedWxid)
  }
  ...
  private emitCacheResolved(payload: { sessionId?: string; imageMd5?: string; imageDatName?: string }, cacheKey: string, localPath: string, isThumb?: boolean): void {
    const message = { cacheKey, imageMd5: payload.imageMd5, imageDatName: payload.imageDatName, localPath, isThumb }
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.isDestroyed()) {
  ...
  private getAllCacheRoots(): string[] {
    ...
    // 默认路径
    roots.push(join(documentsPath, 'ChatCapsule', 'Images'))
    roots.push(join(documentsPath, 'ChatCapsule', 'images'))

    // 兼容旧路径（如果有的话）
    roots.push(join(documentsPath, 'ChatCapsuleData', 'Images'))
  ...
  private getCacheRoot(): string {
    ...
    const root = configured
      ...
      : join(app.getPath('documents'), 'ChatCapsule', 'Images')
    if (!existsSync(root)) {
