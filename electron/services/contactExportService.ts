...
interface ContactExportContact {
    username: string
    displayName?: string
    remark?: string
    nickname?: string
...
class ContactExportService {
     ...
     * 导出为JSON格式
     */
    private async exportToJSON(contacts: ContactExportContact[], outputPath: string): Promise<void> {
        const data = {
     ...
     * 导出为CSV格式
     */
    private async exportToCSV(contacts: ContactExportContact[], outputPath: string): Promise<void> {
     ...
     * 导出为VCF格式（vCard）
     */
    private async exportToVCF(contacts: ContactExportContact[], outputPath: string): Promise<void> {
        ...
        await writeFile(outputPath, vcards.join('\r\n\r\n'), 'utf-8')
    }
