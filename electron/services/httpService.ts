...
import {
  ...
  extractHttpApiToken,
  generateHttpApiToken,
  HTTP_API_TOKEN_HEADER,
  isOriginAllowed,
  maskHttpApiToken,
  normalizeOriginAllowlist,
  resolveMediaRequestPath,
  type HttpApiStatus,
...
const logger = createElectronLogger('HttpService')

class HttpService {
  private server: http.Server | null = null
  private configService: ConfigService
  private port: number = DEFAULT_HTTP_API_PORT
   ...
   * 启动 HTTP 服务
   */
  async start(port: number = DEFAULT_HTTP_API_PORT): Promise<{ success: boolean; port?: number; error?: string }> {
    ...
    return new Promise((resolve) => {
      ...
      this.server.on('error', (err: NodeJS.ErrnoException) => {
        if (err.code === 'EADDRINUSE') {
          ...
          auditService.record({
            event: 'http_api_start_failed',
            severity: 'warn',
            data: { port: this.port, code: err.code, message: err.message }
          })
          ...
          auditService.record({
            event: 'http_api_start_failed',
            severity: 'error',
            data: { port: this.port, code: err.code || 'UNKNOWN', message: err.message }
          })
      ...
      this.server.listen(this.port, '127.0.0.1', () => {
        this.running = true
        logger.info(`HTTP API server started on http://127.0.0.1:${this.port}`)
        auditService.record({
          event: 'http_api_started',
          data: {
            port: this.port,
            allowedOriginCount: this.getAllowedOrigins().length
          }
        })
  ...
  getStatus(): HttpApiStatus {
    ...
    return {
      ...
      port: this.getPort(),
      mediaExportPath: this.getApiMediaExportPath(),
      authRequired: true,
      tokenPresent: token.length > 0,
      tokenMasked: maskHttpApiToken(token),
      allowedOrigins: this.getAllowedOrigins(),
      securityMode: 'strict-local-readonly'
    }
  }

  async copyTokenToClipboard(): Promise<HttpApiTokenView> {
    const token = String(this.configService.get('httpApiToken') || '').trim()
    if (!token) {
      return { tokenPresent: false, tokenMasked: null }
    }
  ...
  rotateToken(): string {
    const token = generateHttpApiToken()
    this.configService.set('httpApiToken', token)
    logger.info('HTTP API token rotated')
    auditService.record({
      event: 'http_api_token_rotated',
      data: {
        tokenLength: token.length,
        allowedOriginCount: this.getAllowedOrigins().length
      }
  ...
  setAllowedOrigins(origins: string[]): string[] {
    const normalized = normalizeOriginAllowlist(origins)
    this.configService.set('httpApiAllowedOrigins', normalized)
    auditService.record({
      event: 'http_api_allowed_origins_updated',
      data: {
        allowedOriginCount: normalized.length,
  ...
  private getAllowedOrigins(): string[] {
    return normalizeOriginAllowlist(this.configService.get('httpApiAllowedOrigins'))
  }

  private getApiToken(): string {
    const currentToken = String(this.configService.get('httpApiToken') || '').trim()
    if (currentToken) return currentToken
    const generatedToken = generateHttpApiToken()
    this.configService.set('httpApiToken', generatedToken)
    logger.info('HTTP API token initialized')
    auditService.record({
      event: 'http_api_token_initialized',
      data: { tokenLength: generatedToken.length }
    })
  ...
  private applyCorsHeaders(req: http.IncomingMessage, res: http.ServerResponse): boolean {
    ...
    res.setHeader('Access-Control-Allow-Headers', `Content-Type, Authorization, ${HTTP_API_TOKEN_HEADER}`)
    res.setHeader('Vary', 'Origin')
    return true
  }

  private isAuthorizedRequest(req: http.IncomingMessage): boolean {
    const token = extractHttpApiToken({
      authorization: req.headers.authorization,
      tokenHeader: req.headers[HTTP_API_TOKEN_HEADER],
    })
  ...
  private async handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    if (!this.applyCorsHeaders(req, res)) {
      this.sendError(res, 403, 'FORBIDDEN', 'Origin is not allowed')
      return
    }
    ...
    if (req.method !== 'GET') {
      this.sendError(res, 405, 'METHOD_NOT_ALLOWED', 'Only GET requests are allowed')
      return
    }

    const url = new URL(req.url || '/', `http://127.0.0.1:${this.port}`)
    const pathname = url.pathname
    const isHealthRoute = pathname === '/health' || pathname === '/api/v1/health'

    if (!isHealthRoute && !this.isAuthorizedRequest(req)) {
      this.sendError(res, 401, 'UNAUTHORIZED', 'Missing or invalid API token')
      return
    }
    ...
    try {
      ...
      if (isHealthRoute) {
        ...
        await this.handleMediaRequest(pathname, res)
      } else {
  ...
  private async handleMediaRequest(pathname: string, res: http.ServerResponse): Promise<void> {
    const mediaBasePath = this.getApiMediaExportPath()
    const relativePath = pathname.replace('/api/v1/media/', '')
    const fullPath = resolveMediaRequestPath(mediaBasePath, relativePath)

    if (!fullPath) {
      this.sendError(res, 400, 'INVALID_MEDIA_PATH', 'Invalid media path')
      return
    }
    try {
      const stats = await fs.promises.stat(fullPath)
      if (!stats.isFile()) {
        this.sendError(res, 404, 'MEDIA_NOT_FOUND', 'Media not found')
        return
      }
      ...
      stream.on('error', () => {
        if (!res.headersSent) {
          this.sendError(res, 500, 'MEDIA_READ_FAILED', 'Failed to read media file')
        } else {
          res.destroy()
        }
      })
  ...
  private async fetchMessagesBatch(
    ...
    try {
      ...
      const cursorResult = await wcdbReadonlyRepository.openMessageCursor(talker, batchSize, ascending, beginTimestamp, endTimestamp)
      ...
      logger.error('fetchMessagesBatch error:', e)
  ...
  private async writeFileIfMissing(fullPath: string, data: Buffer): Promise<void> {
    try {
      await fs.promises.writeFile(fullPath, data, { flag: 'wx' })
    } catch (error) {
    ...
    try {
        ...
        if (result.success && result.localPath) {
          ...
          if (imagePath.startsWith('data:')) {
            ...
            if (base64Match) {
              ...
              await this.writeFileIfMissing(fullPath, imageBuffer)
              const relativePath = `${this.sanitizeFileName(talker, 'session')}/images/${fileName}`
              return { kind: 'image', fileName, fullPath, relativePath }
            }
          } else if (await this.pathExists(imagePath)) {
            const imageBuffer = await fs.promises.readFile(imagePath)
            const ext = this.detectImageExt(imageBuffer)
            const fileBase = this.sanitizeFileName(msg.imageMd5 || msg.imageDatName || `image_${msg.localId}`, `image_${msg.localId}`)
            const fileName = `${fileBase}${ext}`
            const fullPath = path.join(targetDir, fileName)
            await this.copyFileIfMissing(imagePath, fullPath)
            const relativePath = `${this.sanitizeFileName(talker, 'session')}/images/${fileName}`
            return { kind: 'image', fileName, fullPath, relativePath }
          }
        }
      ...
      if (msg.localType === 34 && options.exportVoices) {
        ...
        if (result.success && result.data) {
          ...
          const fullPath = path.join(targetDir, fileName)
          await this.ensureDir(targetDir)
          await this.writeFileIfMissing(fullPath, Buffer.from(result.data, 'base64'))
      ...
      if (msg.localType === 43 && options.exportVideos && msg.videoMd5) {
        ...
        if (info.exists && info.videoUrl && await this.pathExists(info.videoUrl)) {
          ...
          const fullPath = path.join(targetDir, fileName)
          await this.ensureDir(targetDir)
          await this.copyFileIfMissing(info.videoUrl, fullPath)
      ...
      if (msg.localType === 47 && options.exportEmojis && msg.emojiCdnUrl) {
        ...
        if (result.success && result.localPath && await this.pathExists(result.localPath)) {
          ...
          const fullPath = path.join(targetDir, fileName)
          await this.ensureDir(targetDir)
          await this.copyFileIfMissing(result.localPath, fullPath)
          const relativePath = `${this.sanitizeFileName(talker, 'session')}/emojis/${fileName}`
          return { kind: 'emoji', fileName, fullPath, relativePath }
        }
      }
    } catch (e) {
      logger.warn('exportMediaForMessage failed:', e)
    }

    return null
  }

  private toApiMessage(msg: Message, media?: ApiExportedMedia): ApiMessagePayload {
    return {
      localId: msg.localId,
  ...
  private async getDisplayNames(usernames: string[]): Promise<Record<string, string>> {
    try {
      const result = await wcdbReadonlyRepository.getDisplayNames(usernames)
      if (result.success && result.map) {
        return result.map
      }
    } catch (e) {
      logger.error('Failed to get display names:', e)
    }
  ...
  private async convertToChatLab(
    ...
    if (isGroup) {
      try {
        const result = await wcdbReadonlyRepository.getGroupNicknames(talkerId)
        if (result.success && result.nicknames) {
          groupNicknamesMap = new Map(Object.entries(result.nicknames))
        }
      } catch (e) {
        logger.error('Failed to get group nicknames:', e)
      }
    }
   ...
   * 发送错误响应
   */
  private sendError(res: http.ServerResponse, code: number, errorCode: string, message?: string): void {
