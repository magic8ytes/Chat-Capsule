...
class ExportService {
  ...
  private readonly STOP_ERROR_CODE = 'CHATCAPSULE_EXPORT_STOP_REQUESTED'
  ...
  private getChatCapsuleHeader(): { version: string; exportedAt: number; generator: string } {
    return {
      version: '1.0.3',
      exportedAt: Math.floor(Date.now() / 1000),
      generator: 'ChatCapsule'
    }
  }
  ...
  async exportSessionToDetailedJson(
    ...
    try {
      ...
      const capsuleHeader = this.getChatCapsuleHeader()
      ...
      if (options.format === 'arkme-json') {
        ...
        const headerWithFormat = {
          ...capsuleHeader,
          format: 'arkme-json'
        }
        const arkmeExport: any = {
          chatcapsule: headerWithFormat,
          weflow: headerWithFormat,
        ...
        const detailedExport: any = {
          chatcapsule: capsuleHeader,
          weflow: capsuleHeader,
...
`);
    ...
    <script>
      ...
      // Initial Data
      let allData = window.CHATCAPSULE_DATA || [];
        ...
        if (aggregatedData) {
          ...
          const fastRows = await parallelLimit(
            ...
            async (sessionId): Promise<{
              ...
              const totalCount = toCount(metric?.totalMessages)
              const voiceCount = toCount(metric?.voiceMessages)
              const imageCount = toCount(metric?.imageMessages)
              const videoCount = toCount(metric?.videoMessages)
              const emojiCount = toCount(metric?.emojiMessages)
              const lastTimestampValue = metric?.lastTimestamp
              const lastTimestamp = (typeof lastTimestampValue === 'number' && Number.isFinite(lastTimestampValue) && lastTimestampValue > 0)
                ? Math.floor(lastTimestampValue)
                : undefined
      ...
      const runOne = async (sessionId: string): Promise<'done' | 'stopped'> => {
        try {
          ...
          if (canTrySkipUnchanged) {
            const latestRecord = await exportRecordService.getLatestRecord(sessionId, effectiveOptions.format)
          ...
          const isEmptyExport =
            ...
            result.error.includes('没有消息')

          if (isEmptyExport) {
            skipReason = '该会话在指定时间范围内没有消息，已跳过'
            result = { success: true }
          }
          ...
          if (result.success) {
            successCount++
            successSessionIds.push(sessionId)
            if (!skipReason && typeof messageCountHint === 'number' && messageCountHint >= 0) {
            ...
            if (useSessionFolder) {
              try {
                ...
                console.warn(`清理空目录失败: ${sessionDir}`, cleanupError)
              }
            }
          }
          ...
          onProgress?.({
            ...
            currentSessionId: sessionId,
            phase: 'complete',
            phaseLabel: skipReason || (result.success ? '完成' : '导出失败')
          })
          return 'done'
        } catch (error) {
