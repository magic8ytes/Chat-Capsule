...
const logger = createElectronLogger('WasmService')
...
export class WasmService {
        ...
        this.initPromise = new Promise((resolve, reject) => {
            try {
                ...
                mockGlobal.Module = {
                    ...
                    print: (text: string) => logger.debug('[WASM stdout]', text),
                    printErr: (text: string) => logger.error('[WASM stderr]', text)
                };
                ...
                mockGlobal.wasm_isaac_generate = (ptr: number, size: number) => {
                    // logger.debug(`[WasmService] wasm_isaac_generate called: ptr=${ptr}, size=${size}`);
                ...
                logger.error('[WasmService] Failed to initialize WASM:', error);
                reject(error);
            }
        });
    ...
    public async getRawKeystream(key: string, size: number = 131072): Promise<Buffer> {
        ...
        try {
            ...
            logger.error('[WasmService] Error generating raw keystream:', error);
            throw error;
        }
    }
}
