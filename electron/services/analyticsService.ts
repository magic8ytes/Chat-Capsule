...
const logger = createElectronLogger('AnalyticsService')
...
class AnalyticsService {
  ...
  private async getAliasMap(usernames: string[]): Promise<Record<string, string>> {
    ...
    for (let i = 0; i < usernames.length; i += chunkSize) {
      ...
      const result = await wcdbReadonlySqlGateway.execQuery('contact', null, sql)
  ...
  private async iterateSessionMessages(
    ...
    const cursorResult = await wcdbReadonlyRepository.openMessageCursor(
      sessionId,
      500,
      true,
      beginTimestamp,
  ...
  private async getAggregateWithFallback(
    ...
    const promise = (async () => {
      const result = await wcdbReadonlyRepository.getAggregateStats(sessionIds, beginTimestamp, endTimestamp)
  ...
  private getCacheFilePath(): string {
    return join(app.getPath('documents'), 'ChatCapsule', 'analytics_cache.json')
  }
  ...
  private async logAggregateDiagnostics(sessionIds: string[]): Promise<void> {
    ...
    const results = await Promise.all(samples.map(async (sessionId) => {
      const countResult = await wcdbReadonlyRepository.getMessageCount(sessionId)
  ...
  async getExcludeCandidates(): Promise<{ success: boolean; data?: Array<{ username: string; displayName: string; avatarUrl?: string; wechatId?: string }>; error?: string }> {
    try {
      ...
      const [displayNames, avatarUrls, aliasMap] = await Promise.all([
        wcdbReadonlyRepository.getDisplayNames(usernameList),
        wcdbReadonlyRepository.getAvatarUrls(usernameList),
