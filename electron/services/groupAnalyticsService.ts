...
const logger = createElectronLogger('GroupAnalyticsService')
...
class GroupAnalyticsService {
  ...
  private async detectGroupOwnerUsername(
    ...
    try {
      const groupContact = await wcdbReadonlyRepository.getContact(chatroomId)
    ...
    try {
      const roomResult = await wcdbReadonlySqlGateway.execQuery(
        'contact',
        null,
        'SELECT * FROM chat_room WHERE username = ? LIMIT 1',
        [chatroomId]
      )
  ...
  private async ensureConnected(): Promise<{ success: boolean; error?: string }> {
    ...
    if (!decryptKey && !usesMacBackend) return { success: false, error: '未配置解密密钥' }

    const cleanedWxid = this.cleanAccountDirName(wxid)
    const ok = await wcdbConnectionGateway.open(dbPath, decryptKey || '', cleanedWxid)
  ...
  private async getGroupNicknamesForRoom(chatroomId: string, candidates: string[] = []): Promise<Map<string, string>> {
    try {
      ...
      logger.error('getGroupNicknamesForRoom error:', e)
      return new Map<string, string>()
    }
  }
  ...
  private parseGroupNicknamesFromExtBuffer(buffer: Buffer, candidates: string[] = []): Map<string, string> {
    ...
    try {
      ...
      logger.error('Failed to parse chat_room.ext_buffer:', e)
    }
  ...
  private async buildGroupMemberContactLookup(usernames: string[]): Promise<Map<string, GroupMemberContactInfo>> {
    ...
    for (let i = 0; i < candidates.length; i += batchSize) {
      ...
      `
      let result = await wcdbReadonlySqlGateway.execQuery('contact', null, lightweightSql, batch)
      if (!result.success || !result.rows) {
        // 兼容历史/变体列名，轻查询失败时回退全字段查询，避免好友标识丢失
        result = await wcdbReadonlySqlGateway.execQuery('contact', null, `SELECT * FROM contact WHERE username IN (${placeholders})`, batch)
      }
  ...
  private async buildGroupMessageCountLookup(chatroomId: string): Promise<Map<string, number>> {
    ...
    const result = await wcdbReadonlyRepository.getGroupStats(chatroomId, 0, 0)
  ...
  async getGroupChats(): Promise<{ success: boolean; data?: GroupChatInfo[]; error?: string }> {
    try {
      ...
      const [memberCounts, contactInfo] = await Promise.all([
        wcdbReadonlyRepository.getGroupMemberCounts(groupIds),
      ...
      if (!contactInfo.success || !contactInfo.contacts) {
        const [displayNames, avatarUrls] = await Promise.all([
          wcdbReadonlyRepository.getDisplayNames(groupIds),
          wcdbReadonlyRepository.getAvatarUrls(groupIds)
        ])
  ...
  private async loadGroupMembersPanelDataFresh(
    ...
    const membersResult = await wcdbReadonlyRepository.getGroupMembers(chatroomId)
    ...
    const displayNamesPromise = wcdbReadonlyRepository.getDisplayNames(usernames)
  ...
  async getGroupMembers(chatroomId: string): Promise<{ success: boolean; data?: GroupMember[]; error?: string }> {
    try {
      ...
      const result = await wcdbReadonlyRepository.getGroupMembers(chatroomId)
      ...
      await this.parallelLimit(usernames, concurrency, async (username) => {
        const contactResult = await wcdbReadonlyRepository.getContact(username)
  ...
  async getGroupMessageRanking(chatroomId: string, limit: number = 20, startTime?: number, endTime?: number): Promise<{ success: boolean; data?: GroupMessageRank[]; error?: string }> {
    try {
      ...
      const result = await wcdbReadonlyRepository.getGroupStats(chatroomId, startTime || 0, endTime || 0)
      ...
      const [names, avatars] = await Promise.all([
        wcdbReadonlyRepository.getDisplayNames(usernames),
        wcdbReadonlyRepository.getAvatarUrls(usernames)
      ])
  ...
  async exportGroupMemberMessages(
    ...
    try {
      ...
      const exportGenerator = 'Chat Capsule'
      const exportPlatform = 'wechat'

      const groupDisplay = await wcdbReadonlyRepository.getDisplayNames([normalizedChatroomId, normalizedMemberUsername])
  ...
  async exportGroupMembers(chatroomId: string, outputPath: string): Promise<{ success: boolean; count?: number; error?: string }> {
    try {
      ...
      const groupDisplay = await wcdbReadonlyRepository.getDisplayNames([chatroomId])
      const groupName = groupDisplay.success && groupDisplay.map
