...
import { normalizeSnsMediaRemoteUrl, SNS_MEDIA_HOST_ALLOWLIST } from '../../shared/contracts/sns'

const logger = createElectronLogger('SnsService')
...
function parseCommentsFromXml(xml: string): ParsedCommentItem[] {
    ...
    try {
        ...
        logger.error('[SnsService] parseCommentsFromXml 失败:', e)
    }
...
class SnsService {
    ...
    private async resolveContactIdentity(
        ...
        if (!pending) {
            pending = (async () => {
                ...
                try {
                    const contactResult = await wcdbReadonlyRepository.getContact(normalized)
    ...
    private parseLikeUsersFromXml(xml: string): ParsedLikeUser[] {
        ...
        try {
            ...
            logger.error('[SnsService] 解析点赞用户失败:', e)
        }
        return likes
    }
    ...
    private isBackendNotConnectedError(error: unknown): boolean {
        const message = String(error || '').toLowerCase()
        return message.includes('尚未连接') || message.includes('not connected')
    }

    private async ensureBackendConnected(): Promise<{ success: boolean; error?: string }> {
        try {
            const connected = await wcdbConnectionGateway.isConnected()
            if (connected) return { success: true }

            const dbPath = this.toOptionalString(this.configService.get('dbPath')) || ''
            const decryptKey = this.toOptionalString(this.configService.get('decryptKey')) || ''
            const myWxid = this.toOptionalString(this.configService.get('myWxid')) || ''
            const openOk = await wcdbConnectionGateway.open(dbPath, decryptKey, myWxid)
            if (!openOk) {
                return { success: false, error: '数据库尚未连接' }
            }
            return { success: true }
        } catch (error) {
    ...
    private async getExportStatsFromTimeline(myWxid?: string): Promise<{ totalPosts: number; totalFriends: number; myPosts: number | null }> {
        ...
        for (let round = 0; round < 2000; round++) {
            const result = await wcdbReadonlyRepository.getSnsTimeline(pageSize, offset, undefined, undefined, 0, 0)
    ...
    private parseLikesFromXml(xml: string): string[] {
        ...
        try {
            ...
            logger.error('[SnsService] 解析点赞失败:', e)
        }
        return likes
    }
    ...
    private parseMediaFromXml(xml: string): { media: SnsMedia[]; videoKey?: string } {
        ...
        try {
            ...
            logger.error('[SnsService] 解析媒体 XML 失败:', e)
        }
    ...
    private getSnsCacheDir(): string {
        ...
        return join(cachePath, 'sns_cache')
    }
    ...
    async getSnsUsernames(): Promise<{ success: boolean; usernames?: string[]; error?: string }> {
        ...
        const primary = await wcdbReadonlySqlGateway.execQuery(
            'sns',
            null,
            "SELECT DISTINCT user_name FROM SnsTimeLine WHERE user_name IS NOT NULL AND user_name <> ''"
        )
        const fallback = await wcdbReadonlySqlGateway.execQuery(
            'sns',
            null,
    ...
    private async getExportStatsFromTableCount(myWxid?: string): Promise<{ totalPosts: number; totalFriends: number; myPosts: number | null }> {
        ...
        const postCountResult = await wcdbReadonlySqlGateway.execQuery('sns', null, 'SELECT COUNT(1) AS total FROM SnsTimeLine')
        ...
        if (totalPosts > 0) {
            const friendCountPrimary = await wcdbReadonlySqlGateway.execQuery(
                'sns',
                null,
            ...
            if (friendCountPrimary.success && friendCountPrimary.rows && friendCountPrimary.rows.length > 0) {
                ...
                const friendCountFallback = await wcdbReadonlySqlGateway.execQuery(
                    'sns',
                    null,
        ...
        const normalizedMyWxid = this.toOptionalString(myWxid)
        if (normalizedMyWxid) {
            const myPostPrimary = await wcdbReadonlySqlGateway.execQuery(
                'sns',
                null,
            ...
            if (myPostPrimary.success && myPostPrimary.rows && myPostPrimary.rows.length > 0) {
                ...
                const myPostFallback = await wcdbReadonlySqlGateway.execQuery(
                    'sns',
                    null,
    ...
    async getExportStats(options?: {
        ...
        try {
            ...
            const connectResult = await this.ensureBackendConnected()
            if (!connectResult.success) {
                ...
                return { success: false, error: connectResult.error || '数据库尚未连接' }
            }
            ...
            if (
                ...
                try {
                    ...
                    if (!this.isBackendNotConnectedError(error)) {
                        logger.error('[SnsService] getExportStats timeline fallback failed:', error)
                    }
                }
            }
    ...
    async getUserPostCounts(options?: {
        ...
        try {
            ...
            if (!this.isBackendNotConnectedError(error)) {
                logger.error('[SnsService] getUserPostCounts failed:', error)
            }
            if (this.userPostCountsCache) {
                return { success: true, counts: this.userPostCountsCache.counts }
            }
    ...
    async getTimeline(limit: number = 20, offset: number = 0, usernames?: string[], keyword?: string, startTime?: number, endTime?: number): Promise<{ success: boolean; timeline?: SnsPost[]; error?: string }> {
        const result = await wcdbReadonlyRepository.getSnsTimeline(limit, offset, usernames, keyword, startTime, endTime)
        ...
        const enrichedTimeline = result.timeline.map((post: any) => {
            ...
            const parsedMedia = rawXml ? this.parseMediaFromXml(rawXml).media : []
            const mediaSource = Array.isArray(post.media) && post.media.length > 0 ? post.media : parsedMedia

            const fixedMedia = mediaSource.map((m: any) => ({
            ...
            const likesSource = Array.isArray(post.likes) && post.likes.length > 0
                ? post.likes
                : this.parseLikesFromXml(rawXml)
    ...
    async proxyImage(url: string, key?: string | number): Promise<{ success: boolean; dataUrl?: string; videoPath?: string; error?: string }> {
        const normalizedUrl = normalizeSnsMediaRemoteUrl(url)
        if (!normalizedUrl) return { success: false, error: 'SNS 媒体 URL 不合法或不在白名单内' }
        const cacheKey = `${normalizedUrl}|${key ?? ''}`

        if (this.imageCache.has(cacheKey)) {
            return { success: true, dataUrl: this.imageCache.get(cacheKey) }
        }

        const result = await this.fetchAndDecryptImage(normalizedUrl, key)
        if (result.success) {
            // 如果是视频，返回受控本地媒体协议路径
    ...
    async downloadImage(url: string, key?: string | number): Promise<{ success: boolean; data?: Buffer; contentType?: string; error?: string }> {
        const normalizedUrl = normalizeSnsMediaRemoteUrl(url)
        if (!normalizedUrl) {
            return { success: false, error: 'SNS 媒体 URL 不合法或不在白名单内' }
        }
        return this.fetchAndDecryptImage(normalizedUrl, key)
    }
            ...
            if (shouldExportMedia) {
                ...
                const runTask = async (task: typeof mediaTasks[0]) => {
                    ...
                    try {
                        ...
                        logger.warn(`[SnsExport] 媒体下载失败: ${task.url}`, e)
                    }
                    done++
...
<div class="container">
    ...
    ${postsHtml}
    <div class="ft">由 Chat Capsule 导出 · ${new Date().toLocaleString('zh-CN')}</div>
</div>
...
window.addEventListener('scroll',function(){document.getElementById('btt').classList.toggle('show',window.scrollY>600)})
    ...
    private async fetchAndDecryptImage(url: string, key?: string | number): Promise<{ success: boolean; data?: Buffer; contentType?: string; cachePath?: string; error?: string }> {
        ...
        const cachePath = this.getCacheFilePath(normalizedUrl)
        ...
        if (await this.fileExists(cachePath)) {
            try {
                ...
                logger.warn(`[SnsService] 读取缓存失败: ${cachePath}`, e)
            }
        }
                            ...
                            try {
                                ...
                                if (key && String(key).trim().length > 0) {
                                    try {
                                        ...
                                        logger.error(`[SnsService] 视频解密出错: ${err}`)
                                    }
                                }
                                ...
                                logger.error(`[SnsService] 视频处理失败:`, e)
                    ...
                    stream.on('end', async () => {
                        ...
                        if (shouldDecrypt) {
                            try {
                                ...
                                logger.error('[SnsService] TS Decrypt Error:', e)
                            }
                        }
                        ...
                        try {
                            ...
                            logger.warn(`[SnsService] 写入缓存失败: ${cachePath}`, e)
                        }
    ...
    private doDownloadRaw(targetUrl: string, cacheKey: string, cacheDir: string): Promise<string | null> {
        return new Promise((resolve) => {
            let tempPath = join(cacheDir, `${cacheKey}.${Date.now()}.download`)
            let settled = false
            ...
            try {
                ...
                const fixedUrl = normalizeSnsMediaRemoteUrl(targetUrl)?.replace(/&amp;/g, '&') || ''
                const urlObj = new URL(fixedUrl)
                const allowedHost = SNS_MEDIA_HOST_ALLOWLIST.some((suffix) => urlObj.hostname === suffix || urlObj.hostname.endsWith(`.${suffix}`))
                if (!allowedHost || !/^https?:$/i.test(urlObj.protocol)) {
                    finalize(null)
                    return
                }
                ...
                const request = protocol.get(fixedUrl, options, (response: any) => {
                    if ([301, 302, 303, 307].includes(response.statusCode)) {
                        ...
                        if (redirectUrl) {
                            ...
                            response.resume?.()
                            this.doDownloadRaw(full, cacheKey, cacheDir).then(finalize)
                            return
                        }
                    }
                    ...
                    response.on('data', (chunk: Buffer) => {
                        ...
                        if (headerLength >= 64) return
                        const remaining = 64 - headerLength
                        const headerChunk = chunk.subarray(0, Math.min(chunk.length, remaining))
                    ...
                    response.on('error', () => {
                        fileStream.destroy()
                        void cleanupTemp().then(() => finalize(null))
                    })
                    ...
                    fileStream.on('finish', async () => {
                        try {
                            ...
                            const ext = this.isValidImageBuffer(headerBuffer) ? this.getImageExtFromBuffer(headerBuffer) : '.bin'
    ...
    async downloadSnsEmoji(url: string, encryptUrl?: string, aesKey?: string): Promise<{ success: boolean; localPath?: string; error?: string }> {
        ...
        for (const ext of ['.gif', '.png', '.webp', '.jpg', '.jpeg']) {
            ...
            if (await this.fileExists(filePath)) return { success: true, localPath: filePath }
        }
        ...
        if (encryptUrl && aesKey) {
            ...
            if (encResult) {
                const encData = await readFile(encResult)
                if (this.isValidImageBuffer(encData)) {
                    const ext = this.getImageExtFromBuffer(encData)
                    const filePath = join(emojiDir, `${cacheKey}${ext}`)
                    await writeFile(filePath, encData)
                    await unlink(encResult).catch(() => undefined)
                    return { success: true, localPath: filePath }
                }
