export interface ParsedChatRecordItem {
  datatype: number
  sourcename: string
  sourcetime: string
...
const buildChatRecordItem = (xml: string, datatypeHint?: number): ParsedChatRecordItem | null => {
  if (!xml) return null
...
const parseChatRecordDataItems = (xml: string): ParsedChatRecordItem[] => {
  if (!xml) return []

  const items: ParsedChatRecordItem[] = []
...
export const parseChatRecordList = (content: string): ParsedChatRecordItem[] | undefined => {
  const decodedContent = decodeHtmlEntitiesDeep(content) || content
  const type = extractXmlValue(content, 'type') || extractXmlValue(decodedContent, 'type')
  if (type !== '19') return undefined

  const itemKeys = new Set<string>()
  const recordList: ParsedChatRecordItem[] = []
  ...
  const pushItem = (item: ParsedChatRecordItem | null | undefined) => {
    if (!item) return
    const key = [
      item.messageuuid || '',
      item.datatype,
