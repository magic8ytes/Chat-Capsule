...
export const Avatar = React.memo(function Avatar({
    ...
    const isCached = useMemo(() => normalizedSrc ? loadedAvatarCache.has(normalizedSrc) : false, [normalizedSrc])
    ...
    useEffect(() => {
        if (!lazy || shouldLoad || isInQueue || !normalizedSrc || !containerRef.current || isCached) return
            ...
            (entries) => {
                entries.forEach((entry) => {
                    if (entry.isIntersecting && !isInQueue) {
                        setIsInQueue(true)
                        avatarLoadQueue.enqueue(normalizedSrc).then(() => {
                            setShouldLoad(true)
                        }).catch(() => {
    ...
    }, [normalizedSrc, lazy, shouldLoad, isInQueue, isCached])

    // Reset state when src changes
    useEffect(() => {
        const cached = normalizedSrc ? loadedAvatarCache.has(normalizedSrc) : false
    ...
    const hasValidUrl = !!normalizedSrc && !imageError && shouldLoad
            ...
            {hasValidUrl ? (
                <>
                    ...
                    <img
                        ...
                        onLoad={() => {
                            if (normalizedSrc) loadedAvatarCache.add(normalizedSrc)
                            setImageLoaded(true)
                        }}
