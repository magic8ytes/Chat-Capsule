...
import { sns } from '../../services/ipc'
import { createLogger } from '../../utils/logger'
import { toSafeMediaUrl } from '../../utils/mediaUrl'

const logger = createLogger('SnsMediaGrid')
        ...
        const load = async () => {
            try {
                if (!isVideo) {
                    ...
                    const result = await sns.proxyImage({
                        url: targetUrl,
                    ...
                    if (result.success) {
                        ...
                        else if (result.videoPath) setThumbSrc(toSafeMediaUrl(result.videoPath) || '')
                    } else {
                        markDeleted()
                    }
                    ...
                    if (isLive && media.livePhoto?.url) {
                        sns.proxyImage({
                            ...
                            if (!cancelled && res.success && res.videoPath) {
                                setLiveVideoPath(toSafeMediaUrl(res.videoPath) || '')
                            }
                        }).catch(() => { })
                    }
                    setLoading(false)
                    ...
                    if (result.success && result.videoPath) {
                        const localPath = toSafeMediaUrl(result.videoPath) || ''
    ...
    const handlePreview = async (e: React.MouseEvent) => {
        ...
        if (isVideo) {
            ...
            if (!videoPath) {
                ...
                try {
                    const res = await sns.proxyImage({
                        url: media.url,
                        key: skipDecrypt ? undefined : media.key
                    })
                    if (res.success && res.videoPath) {
                        const local = toSafeMediaUrl(res.videoPath) || ''
                        setVideoPath(local)
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.dataUrl) {
                    ...
                    try {
                        const response = await fetch(toSafeMediaUrl(result.videoPath) || '')
                        const blob = await response.blob()
                        const url = URL.createObjectURL(blob)
                        link.href = url
                        setTimeout(() => URL.revokeObjectURL(url), 60000)
                    } catch (err) {
                        logger.error('Video fetch failed, falling back to direct link', err)
                        link.href = toSafeMediaUrl(result.videoPath) || ''
                    }
                }
