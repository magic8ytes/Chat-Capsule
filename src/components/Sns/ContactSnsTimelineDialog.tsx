...
import { formatYmdDateFromSeconds } from '../../utils/formatters'
import { createLogger } from '../../utils/logger'
import { AvatarImage } from '../AvatarImage'
import './ContactSnsTimelineDialog.scss'

const logger = createLogger('ContactSnsTimelineDialog')
...
export function ContactSnsTimelineDialog({
  ...
  const loadTimelinePosts = useCallback(async (nextTarget: ContactSnsTimelineTarget, options?: { reset?: boolean }) => {
    ...
    try {
      ...
      const result = await electronApi.sns.getTimeline(
        TIMELINE_PAGE_SIZE,
        0,
        [nextTarget.username],
        '',
  ...
  const loadTimelineTotalPosts = useCallback(async (nextTarget: ContactSnsTimelineTarget) => {
    ...
    try {
      const result = await electronApi.sns.getUserPostCounts()
        ...
        <div className="contact-sns-dialog-header">
          <div className="contact-sns-dialog-header-main">
            <div className="contact-sns-dialog-avatar">
              <AvatarImage src={targetAvatarUrl} name={targetDisplayName} alt="" loading="eager" />
            </div>
              ...
              {timelinePosts.map((post) => (
                <SnsPostItem
                  ...
                  onPreview={(src, isVideo, liveVideoPath) => {
                    if (isVideo) {
                      void electronApi.window.openVideoPlayerWindow(src)
                    } else {
                      void electronApi.window.openImageViewerWindow(src, liveVideoPath || undefined)
                    }
                  }}
