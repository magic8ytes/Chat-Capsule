...
import { Search, RefreshCw, X, User, Users, MessageSquare, Loader2, FolderOpen, Download, ChevronDown, MessageCircle, UserX, Aperture } from 'lucide-react'
import { useChatStore } from '../stores/chatStore'
import { toContactTypeCardCounts, useContactTypeCountsStore } from '../stores/contactTypeCountsStore'
import * as configService from '../services/config'
import { chat, dialog, exportApi, sns } from '../services/ipc'
import { createLogger } from '../utils/logger'
import type { ContactInfo } from '../types/models'
import { ContactSnsTimelineDialog } from '../components/Sns/ContactSnsTimelineDialog'
import { AvatarImage } from '../components/AvatarImage'
import { ContactsListMeta } from '../components/contacts/ContactsListMeta'
import { ContactsLoadIssueCard } from '../components/contacts/ContactsLoadIssueCard'
...
function ContactsPage() {
    ...
    const upsertAvatarCacheFromContacts = useCallback((
        ...
        void configService.setContactsAvatarCache(scopeKey, nextCache).catch((error) => {
            logger.error('写入通讯录头像缓存失败:', error)
        })
    }, [])
    ...
    const enrichContactsInBackground = useCallback(async (
        ...
        for (let i = 0; i < total; i += AVATAR_ENRICH_BATCH_SIZE) {
            ...
            try {
                const avatarResult = await chat.enrichSessionsContactInfo(batch)
                if (loadVersionRef.current !== loadVersion) return
    ...
    const loadSnsUserPostCounts = useCallback(async (options?: { force?: boolean }) => {
        ...
        try {
            ...
            logger.error('加载通讯录联系人朋友圈条数失败:', error)
            setSnsUserPostCountsStatus('error')
        }
    ...
    const startExport = async () => {
        ...
        try {
            ...
            const result = await exportApi.exportContacts(exportFolder, exportOptions)
        ...
        <div className="contacts-page">
            ...
            <div className="contacts-panel">
                ...
                <ContactsListMeta
                    ...
                    contactsUpdatedAtLabel={contactsUpdatedAtLabel}
                    contactsDataSource={contactsDataSource}
                    avatarCachedCount={avatarCachedCount}
                    avatarCacheUpdatedAtLabel={avatarCacheUpdatedAtLabel}
                    isLoading={isLoading}
                    ...
                    onToggleAllFiltered={toggleAllFilteredSelected}
                />

                {contacts.length === 0 && loadIssue ? (
                    <ContactsLoadIssueCard
                        loadIssue={loadIssue}
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <div className="contact-avatar">
                                            <AvatarImage src={contact.avatarUrl} name={contact.displayName} alt="" loading="lazy" />
                                        </div>
                    ...
                    <div className="settings-content">
                        <div className="detail-profile">
                            <div className="detail-avatar">
                                <AvatarImage src={selectedContact.avatarUrl} name={selectedContact.displayName} alt="" loading="eager" />
                            </div>
