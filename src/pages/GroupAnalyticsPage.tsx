...
import { app, dialog, exportApi, groupAnalytics } from '../services/ipc'
...
function GroupAnalyticsPage() {
  const logger = createLogger('GroupAnalyticsPage')
  ...
  const loadGroups = useCallback(async () => {
    setIsLoading(true)
    try {
      const result = await groupAnalytics.getGroupChats()
  ...
  const loadFunctionData = async (func: AnalysisFunction) => {
    ...
    try {
      switch (func) {
        case 'members': {
          const result = await groupAnalytics.getGroupMembers(selectedGroup.username)
        ...
        case 'ranking': {
          const result = await groupAnalytics.getGroupMessageRanking(selectedGroup.username, 20, startTime, endTime)
          if (result.success && result.data) setRankings(result.data)
          break
        }
        case 'activeHours': {
          const result = await groupAnalytics.getGroupActiveHours(selectedGroup.username, startTime, endTime)
          if (result.success && result.data) setActiveHours(result.data.hourlyDistribution)
          break
        }
        case 'mediaStats': {
          const result = await groupAnalytics.getGroupMediaStats(selectedGroup.username, startTime, endTime)
  ...
  const handleExportMembers = async () => {
    ...
    try {
      ...
      const result = await groupAnalytics.exportGroupMembers(selectedGroup.username, saveResult.filePath)
      if (result.success) {
